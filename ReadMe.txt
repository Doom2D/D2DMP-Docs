Doom 2D Multiplayer 0.6 (Сборка 126)
=*=*=*=*=*=*=*=*=*=
...
Доступно три режима игры:
	...
	Capture The Flag (CTF) - суть: крадём флаги, отбиваемся от преследования, приносим флаги, прожигаем жизнь.
...
СБОРКА 126:
	...
	- мастерсервер перенесён на другой хост (94.19.235.229:25667);
...
СБОРКА 125:
	...
	- серверлист на PHP-скриптах заменён на полноценный мастерсервер;
	- сервер теперь получает свой внешний IP с мастерсервера;
	- размер игрока изменён на 33x52;
	- бутылёк и черепок теперь дают по 4 HP и 5 AP соответственно;
	- добавлены красные вспышки экрана при боли;
	- добавлена возможность смены используемого сервером порта;
	- добавлена вертикальная синхронизация в клиенте и возможность её отключения;
	...
	- теперь при mp_flymode = 1 физика полёта такая же, как и при обычном джетпаке;
	...
	- прочие исправления/добавления различной степени важности, общее количество которых не поддаётся какому-либо исчислению.
*-*-*-*-*-*-*
СБОРКА 124:
	//сервисная сборка
...
Тестеры: Чёрный Думер, ar888, Jabberwock
